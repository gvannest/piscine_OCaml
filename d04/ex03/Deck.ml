...
let toStringList (deck:t) =
    let rec stringList_aux (deck_aux:t) (result:string list) = match deck_aux with
        | [] -> List.rev result
        | h :: t -> stringList_aux t ((Card.toString h) :: result)
    in
    stringList_aux deck []
...
let toStringListVerbose (deck:t) =
    let rec stringList_aux (deck_aux:t) (result:string list) = match deck_aux with
        | [] -> List.rev result
        | h :: t -> stringList_aux t ((Card.toStringVerbose h) :: result)
    in
    stringList_aux deck []

let drawCard (deck:t) = match deck with
    | [] -> raise (Failure "Error: empty list")
    | h :: n :: t -> (List.hd deck, List.tl deck)
    | h :: t -> (List.hd deck, [])
